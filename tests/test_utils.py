...
# testing error is raised
@patch("src.utils.sqlalchemy_create_engine") # mocking create_engine
def test_init_engine_success(mock_engine):
    mock_engine.return_value = "mock_engine" # return this when called
    result = init_engine("mock-url", echo=False) # fake url
    assert result == "mock_engine" 
    mock_engine.assert_called_once_with("mock-url", echo=False)

# testing error is not raised with side effect
